...
@Component({
  selector: 'app-syn-login',
  templateUrl: './syn-login.component.html',
  styleUrls: ['./syn-login.component.css']
})
export class SynLoginComponent implements OnInit {
  loginForm: FormGroup;
  loginDetails: any = {};
  ipAddress:any;
    ...
    let jsonObj = {
      ...
      this.authService.postApiCall("/api/V1/Accounts/Login", jsonObj).subscribe((data:any) => {
        ...
        if(data.iSsuccessResponse == true )
        {
          ...
          {
            if(data.userData.userTypeId==6||data.userData.userTypeId==7)
            {
            ...
            else
      {
        this.toastr.error('Invalid MailId/Password!');
            ...
            this.toastr.success(data.response);
            this.route.navigate(['/synergy/two-factor'], { state: {userName: this.loginDetails.UserName,password:this.loginDetails.Password} });
          }
          else{
